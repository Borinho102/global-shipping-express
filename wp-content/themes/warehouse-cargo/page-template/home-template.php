...
<main id="skip-content">
  <section id="top-slider">
    <?php if(get_theme_mod('warehouse_cargo_slider_setting') != ''){ ?>
    <?php $warehouse_cargo_slide_pages = array();
      for ( $count = 1; $count <= 3; $count++ ) {
        $warehouse_cargo_mod = intval( get_theme_mod( 'warehouse_cargo_top_slider_page' . $count ));
        if ( 'page-none-selected' != $warehouse_cargo_mod ) {
          $warehouse_cargo_slide_pages[] = $warehouse_cargo_mod;
        }
      }
      if( !empty($warehouse_cargo_slide_pages) ) :
        $warehouse_cargo_args = array(
          'post_type' => 'page',
          'post__in' => $warehouse_cargo_slide_pages,
          'orderby' => 'post__in'
        );
        $warehouse_cargo_query = new WP_Query( $warehouse_cargo_args );
        if ( $warehouse_cargo_query->have_posts() ) :
          $i = 1;
    ?>
    <div class="owl-carousel" role="listbox">
      <?php  while ( $warehouse_cargo_query->have_posts() ) : $warehouse_cargo_query->the_post(); ?>
...
<?php if(get_theme_mod('warehouse_cargo_home_contact_setting') != ''){ ?>
...
<?php if(get_theme_mod('warehouse_cargo_latest_post_setting') != ''){ ?>
      ...
      <div class="row mt-4">
        <?php $warehouse_cargo_other_latest_post_section = array();
          ...
          for ($i=1; $i <= $latest_post_loop; $i++) {
            $warehouse_cargo_mod = intval( get_theme_mod( 'warehouse_cargo_other_latest_post_section' .$i));
            if ( 'page-none-selected' != $warehouse_cargo_mod ) {
              $warehouse_cargo_other_latest_post_section[] = $warehouse_cargo_mod;
            }
          }
